...
class Controls {
    ...
    queryController() {
        ...
        const isButtonPressed = (button: XboxControllerButton) =>
        ...
        const deadzone = 0.1
        if (
            Math.hypot(this.inputDirection.x, this.inputDirection.y) < deadzone
        ) {
        ...
        this.isEscape = Boolean(
            this.keyMap.get("Escape") ||
                isButtonPressed(XboxControllerButton.Select),
        )
    }

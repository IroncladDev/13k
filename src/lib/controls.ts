...
export default class Controls {
    ...
    static init() {
        window.addEventListener("keydown", event => Controls.keys.set(event.key, true))
        window.addEventListener("keyup", event => Controls.keys.delete(event.key))
        ...
        c2d.addEventListener("mousemove", (event: any) => {
            ...
            Controls.mouseX = ((event.clientX - rect.left) * (c2d.width / rect.width)) / canvas.dpr
            Controls.mouseY = ((event.clientY - rect.top) * (c2d.height / rect.height)) / canvas.dpr
        })
    }
